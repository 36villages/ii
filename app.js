...
import { GLTFLoader } from "https://esm.sh/three@0.172.0/examples/jsm/loaders/GLTFLoader";
import * as CameraUtils from "https://esm.sh/three@0.172.0/examples/jsm/utils/CameraUtils";
import { gsap, Power4 } from "https://esm.sh/gsap";
import { EventEmitter } from "https://esm.sh/events";
import OBSWebSocket from "https://esm.sh/obs-websocket-js"; // For OBS integration
...
class App {
  ...
  async connectOBS() {
    try {
      await this.obs.connect("ws://localhost:4455", "your-obs-websocket-password");
  ...
  createWorlds() {
    ...
    this.desertWorld.addListener("moveToPortalComplete", () => this.onMoveToPortalComplete("ForestScene"));
    this.forestWorld.addListener("moveToPortalComplete", () => this.onMoveToPortalComplete("DesertScene"));
  ...
  createParticles() {
    // Add particle system around portals for visual flair
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      colors[i * 3] = 0.5;
      colors[i * 3 + 1] = 0.7;
      colors[i * 3 + 2] = 1.0; // Blueish particles
    }
    ...
    this.currentWorld.scene.add(this.particleSystem);
  }
  ...
  switchWorlds() {
    ...
    this.currentWorld.scene.add(this.particleSystem); // Move particles to new world
    this.onWindowResize();
  }
  ...
  onMoveToPortalComplete(sceneName) {
    this.switchWorlds();
    this.switchOBSScene(sceneName);
    this.currentWorld.moveWorldAndCameraToOrigin();

    // Fade effect
    const fade = document.createElement("div");
    fade.style.cssText = "position:fixed;top:0;left:0;width:100%;height:100%;background:black;opacity:0;pointer-events:none;";
  ...
  loop() {
    ...
    if (this.particleSystem) {
      this.particleSystem.rotation.y += this.deltaTime * 0.1;
    }
...
class World extends EventEmitter {
  ...
  moveWorldToEnd() {
    ...
    gsap.to(this.holder.position, { duration, ease, x: this.endPosition.x, y: this.endPosition.y, z: this.endPosition.z });
    gsap.to(this.holder.scale, { duration, ease, x: this.endScale.x, y: this.endScale.y, z: this.endScale.z });
    gsap.to(this.holder.quaternion, { duration, ease, x: this.endQuaternion.x, y: this.endQuaternion.y, z: this.endQuaternion.z, w: this.endQuaternion.w });
  }
  ...
  moveWorldAndCameraToOrigin() {
    ...
    gsap.to(this.camera.position, {
      ...
      onComplete: () => this.emit("moveToOriginComplete")
    });
  }
  ...
  moveCameraToPortal() {
    ...
    gsap.to(this.camera.position, {
      ...
      z: pos.z,
      onUpdate: () => this.camera.lookAt(this.cameraTarget.position),
      onComplete: () => this.emit("moveToPortalComplete")
    });
    gsap.to(this.portal, {
      duration,
      ease: Power4.easeIn,
      effectIntensity: 0,
      onComplete: () => this.portal.effectIntensity = 1
    });
  }
}
